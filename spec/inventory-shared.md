...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 27 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Outer shadow | Partial/native, including calibrated single-path SVG `drop-shadow()` and schema-ordered blur-plus-shadow lists | Native with exact typed HTML payload and path-aware custom-geometry CSS | `cap:effects` + `cap:custom-path-effects` + `cap:compound-effect-list` (both), LO + Graph evidence | adversarial negative spread, multiple shadows/`effectDag`, and broader color models |
...
| Blur | Native PowerPoint effect retained beneath an isolated portable layer; unique compound `effectLst` children serialize in schema order | Native effect and fallback recovered as one hybrid; CSS plus exact typed payload | `cap:blur-effect` + `cap:compound-effect-list` (both), LO + Graph evidence | `effectDag`, duplicate/arbitrary effect graphs, image/custom geometry cases, and more renderer versions |
...
| Other effect-list constructs | Supported unique `effectLst` effects serialize in fixed ECMA order with exact-count gates; arbitrary graphs remain a gap | Supported `effectLst` subset is typed; other nodes preserve only | `cap:compound-effect-list` (both) + serializer/checker units | `effectDag` containers, duplicate effects, remaining visual fallbacks, and graph re-emission |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 23 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
