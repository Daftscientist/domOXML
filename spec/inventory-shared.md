...
Audited on **2026-07-28**. `cap:*` refers to a manifest below `capabilities/pptx/`; “both” means the
...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 40 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Linear/radial gradients | Native subset; aspect-corrected DrawingML projection and premultiplied-sRGB stop subdivision are shared by shape fills and visually proven linear fill overlays; radial overlay is focused-unit only | Native subset; projected linear overlays recover typed CSS and guarded authored intent without repeated-cycle stop expansion; radial overlays recover typed IR in focused tests | fidelity corpus + `cap:gradient-fill-overlay` (both) + radial units, LO + Graph evidence for linear | every gradient form, radial overlay atomic proof, transforms/stops, theme-aware stops, focal/path variants, and broader renderer calibration |
...
| Fill overlay | Native PowerPoint solid or linear-gradient overlay with a shape-bound portable fallback selected for incompatible renderers for CSS multiply/screen/darken/lighten, including one picture base layer and a strict clipped solid-sRGB SVG graph for custom paths; radial gradients are typed/unit-proven, while partial geometry, active lower blends, changed projections/filter graphs, ambiguous fills, stale metadata, and unsupported blends use the element-layer or preserved-source path | Four proven blend modes recover solid or gradient typed CSS plus exact payload and one hybrid with stable picture/custom-path ownership; guarded gradient intent prevents projected stop expansion; admitted isolated square-cornered opaque `over` rectangles retain a masked owned layer and exact source in `AlternateContent`, while inseparable crops report rasterized/noneditable | `cap:fill-overlay-effect` + `cap:gradient-fill-overlay` + `cap:picture-fill-overlay` + `cap:custom-path-fill-overlay` (both) + radial units + `cap:fill-overlay-owned-fallback` (reverse, two normalized HTML cycles) + overlapping/rounded-crop regressions + Aspose real deck, LO + Graph evidence for proven cases | radial overlay atomic proof, calibrate `over`, pattern overlays, compound ordering, additional custom-geometry gradient/filter forms, and more renderer versions |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 34 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and five two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
