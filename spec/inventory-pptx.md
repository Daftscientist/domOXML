...
| Pictures and raster fallback markers | Native; alpha-preserving isolated effect fallbacks hide their native metadata shape when both coexist in the PowerPoint choice, preventing transparent pixels from double-painting effects. After Graph/LibreOffice calibration, pattern fill overlays and the strict CSS-normal/typed-`over` subset use a hidden native shape plus visible picture in the PowerPoint `mc:Choice`, while the incompatible-renderer `mc:Fallback` contains only the visible picture; repeated mixed-effect slots retain a schema-valid editable subset beneath exact paint; native sibling-effect graphs, sole-visual source-effect fallbacks, and one whole-sequence slide fallback remain renderer-selected through `mc:AlternateContent` | Native; guarded private `over` intent is recovered only from the generated two-visual choice while its native projection matches; authoritative isolated crops attach to preserved positioned nodes, safe composite-only fallbacks report rasterized/noneditable, and normalized HTML recovers slide-owned pixels above retained native contents | capabilities/integration + transparent-isolation regression + `cap:blur-effect` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` + `cap:nested-effect-dag-fallback` + `cap:reflection-effect` + `cap:soft-edge-effect` + `cap:fill-overlay-effect` + `cap:pattern-fill-overlay` + `cap:over-fill-overlay` + `cap:fill-overlay-owned-fallback` + `cap:preset-shadow-raster-fallback` + `cap:preset-shadow-slide-fallback` + malformed slide-marker regression + `cap:chart-preservation` | automatic renderer policy beyond proven effects, multiple unsupported siblings, smaller source-effect isolation, stable group ownership and accessibility fields |
...
| `mixed-shadow-owned-fallback` | both | two outer shadows plus one inset retain exact visible paint in one owned layer, a legal editable native subset, full typed authored intent, stale-projection rejection, exact raster bounds, and two-cycle convergence |
...
| `native-group-picture-roundtrip` | reverse | one cropped native picture child retains its media, crop, identity, ownership, child order, group/child coordinate spaces, exact PowerPoint re-emission, and two-cycle convergence |
...
| `pattern-fill-overlay` | both | one calibrated opaque horizontal multiply pattern retains typed editable `a:pattFill` metadata beneath one exact shape-owned picture in both renderer branches, rejects uncalibrated repeating gradients/presets/blends, and converges after two cycles |
