...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 42 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Fill overlay | Native PowerPoint solid, linear-gradient, or centered-radial metadata uses a shape-bound portable fallback selected for incompatible renderers for CSS multiply/screen/darken/lighten, including one picture base layer and a strict clipped solid-sRGB SVG graph for custom paths. One calibrated opaque horizontal multiply pattern retains hidden editable native `a:pattFill` metadata beneath one exact picture in the PowerPoint choice and uses that picture alone for incompatible renderers because Graph tile edges differ and LibreOffice omits the effect; partial geometry, active lower blends, non-centered radial forms, uncalibrated patterns, changed projections/filter graphs, ambiguous fills, stale metadata, and unsupported blends use the element-layer or preserved-source path | Four proven blend modes recover solid or gradient CSS; the calibrated pattern subset recovers horizontal multiply CSS. Both retain exact payload and one hybrid with stable picture/custom-path ownership; guarded gradient intent prevents projected stop expansion; repeating gradients cannot degrade into finite gradients; admitted isolated square-cornered opaque `over` rectangles retain a masked owned layer and exact source in `AlternateContent`, while inseparable crops report rasterized/noneditable | `cap:fill-overlay-effect` + `cap:gradient-fill-overlay` + `cap:radial-gradient-fill-overlay` + `cap:pattern-fill-overlay` + `cap:picture-fill-overlay` + `cap:custom-path-fill-overlay` (both) + `cap:fill-overlay-owned-fallback` (reverse, two normalized HTML cycles) + overlapping/rounded-crop regressions + Aspose real deck, LO + Graph evidence | calibrate `over`, broader pattern presets and blends, compound ordering, non-centered/focal radial forms, additional custom-geometry gradient/filter forms, and more renderer versions |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 36 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and five two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
