...
| `over-fill-overlay` | both | strict opaque solid base plus full-coverage translucent CSS normal overlay retains guarded editable `a:fillOverlay blend="over"` intent beneath one exact shape-owned picture selected by both renderer branches, rejects unproven paint/base/geometry, rejects stale intent after native edits, and converges after two cycles |
