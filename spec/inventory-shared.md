...
| Outer shadow | Partial/native, including signed spread, calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, and multiple CSS shadows as one editable flat sibling `effectDag` with a transparent portable isolated layer | Native with inverse CSS blur calibration, signed spread recovery, exact typed HTML container/source-reference payload, CSS paint-order recovery, and path-aware custom-geometry CSS | `cap:negative-shadow-spread` + `cap:effects` + `cap:custom-path-effects` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both), LO + Graph evidence | mixed/inset graph branches, thick/compound line alignment, and broader color models |
