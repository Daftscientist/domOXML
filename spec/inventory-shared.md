...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 32 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Outer shadow | Partial/native, including signed spread, calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, one outer-plus-inner `effectLst`, and multiple CSS outer shadows as one editable flat sibling `effectDag` | Native with inverse CSS blur calibration, signed spread recovery, exact typed HTML container/source-reference payload, CSS paint-order recovery across schema ordering, and path-aware custom-geometry CSS | `cap:negative-shadow-spread` + `cap:effects` + `cap:mixed-shadow-effect-list` + `cap:custom-path-effects` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both), LO + Graph evidence | repeatable/more-than-two mixed graphs, thick/compound line alignment, and broader color models |
| Inner shadow | Hybrid editable `a:innerShdw` plus one shape-bound portable fallback, alone or paired with one outer shadow; exact signed CSS intent and authored order are retained in private metadata and ignored when stale after an Office edit | Native read, browser-visible inset CSS, exact intent/order recovery, and regenerated portable fallback | `cap:effects` + `cap:mixed-shadow-effect-list` (both) + native-projection mutation units, LO + Graph evidence | repeatable/more-than-two mixed graphs, pictures/custom geometry, and more renderer versions |
...
| Effect containers and other constructs | Supported unique `effectLst` effects serialize in fixed ECMA order, including one inner-plus-outer pair; repeatable outer shadows serialize as a flat sibling `effectDag` in reversed paint order with a final `fill` or `fillLine` source reference | Supported `effectLst` and flat sibling-shadow graph subsets are typed; authored order survives schema-order normalization through guarded intent; arbitrary/nested graphs preserve as one attached container and use a source-owned renderer fallback when supplied | `cap:compound-effect-list` + `cap:mixed-shadow-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both) + `cap:nested-effect-dag-fallback` (reverse) + arbitrary-graph fallback units, LO + Graph evidence | typed/editable heterogeneous tree graphs, duplicate non-shadow effects, repeatable mixed graphs, smaller source-owned isolation, and representative real decks |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 27 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and four two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
