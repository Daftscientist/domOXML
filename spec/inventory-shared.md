...
Audited on **2026-07-27**. `cap:*` refers to a manifest below `capabilities/pptx/`; “both” means the
...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 30 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Outer shadow | Partial/native, including calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, and multiple CSS shadows as one editable flat sibling `effectDag` with a transparent portable isolated layer | Native with exact typed HTML container/source-reference payload, CSS paint-order recovery, and path-aware custom-geometry CSS | `cap:effects` + `cap:custom-path-effects` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both), LO + Graph evidence | adversarial negative spread, mixed/inset graph branches, thick/compound line alignment, and broader color models |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 25 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and four two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
