...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 29 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Outer shadow | Partial/native, including calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, and multiple CSS shadows as one editable flat sibling `effectDag` with a portable isolated layer | Native with exact typed HTML container/source-reference payload, CSS paint-order recovery, and path-aware custom-geometry CSS | `cap:effects` + `cap:custom-path-effects` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` (both), LO + Graph evidence | adversarial negative spread, mixed/inset graph branches, line-source visual fixtures, and broader color models |
...
| Effect containers and other constructs | Supported unique `effectLst` effects serialize in fixed ECMA order; repeatable outer shadows serialize as a flat sibling `effectDag` in reversed paint order with a final source reference | Supported `effectLst` and flat sibling-shadow graph subsets are typed; arbitrary/nested graphs preserve as one attached container and use a source-owned renderer fallback when supplied | `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` (both) + `cap:nested-effect-dag-fallback` (reverse) + arbitrary-graph fallback units, LO + Graph evidence | typed/editable heterogeneous tree graphs, duplicate non-shadow effects, smaller source-owned isolation, and representative real decks |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 24 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and four two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
