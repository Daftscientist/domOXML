...
Audited on **2026-08-03**. `cap:*` refers to a manifest below `capabilities/pptx/`; “both” means the
...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 45 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
| Units, page boxes, identity, and fixed layout | Native, canonical ordered scene with slide-scoped IDs/provenance | Native, canonical ordered scene with recovered IDs/provenance | `cap:interleaved-order` + `cap:node-identity` (both) + `cap:native-group-roundtrip` + `cap:native-group-picture-roundtrip` (reverse) + broad tests | broader group reconstruction and preservation ownership beyond charts and strict native groups |
...
| Custom paths | Partial/native; single-path SVG drop shadow/glow effects are native, while strict lone SVG blur, below-reflection, path-aware soft edge, and clipped solid fill overlay are hybrid native/exact | Partial/native; DrawingML guides and elliptical arcs normalize to canonical path commands, with path-aware effect HTML, exact typed geometry/effect metadata, and stable exact owned layers for mixed shadows, blur, reflection, soft edge, and fill overlay | `cap:custom-path` + `cap:custom-path-effects` + `cap:custom-path-blur` + `cap:custom-path-reflection` + `cap:custom-path-soft-edge` + `cap:custom-path-fill-overlay` (both) + `cap:custom-path-owned-fallback` (reverse) + Apache POI real deck, LO + Graph evidence | authored SVG arcs/shorthand, multipath, guide/source-formula retention, multi-path fill/stroke semantics, remaining effect families and compound filters |
...
| Groups and stacking | Native `GroupNode` output for nonempty modeled groups, including nested groups, connectors, pictures, and portable child fallbacks; ordinary authored DOM grouping remains flat, while valid direct IR groups outside the strict HTML boundary visibly lower through a flattened wrapper with a warning and recover original geometry over two normalized-browser cycles; a wrapper/member freshness guard leaves visibly edited/stale geometry or transforms flattened with explicit debt, and invalid extents fail explicitly | The proven strict single-level untransformed subset retains native `p:grpSp`, child coordinates, IDs, ownership, surrounding z-order, and a mixed group of native shapes plus cropped native picture children, including each picture's crop data and bound media relationship, through normalized HTML; transformed, nested, connector/fallback-bearing, and otherwise unsupported source groups retain attached source under one slide-owned rasterized/noneditable fallback when caller-rendered pixels exist, or explicitly fail paint coverage without them; additional unsupported owners sharing that raster remain explicit debt | `cap:native-group-roundtrip` + `cap:native-group-picture-roundtrip` (reverse) + Apache POI grouped custom-path real deck + unit/integration + LibreOffice/Graph evidence | arbitrary authored DOM grouping, source nested transforms/connectors/graphic frames, portable-fallback source admission, and independently owned fallbacks where isolation is proved |
| Solid and no fill | Native | Native | integration | explicit atomic bidirectional fixture |
| Linear/radial gradients | Native subset; aspect-corrected DrawingML projection and premultiplied-sRGB stop subdivision are shared by shape fills and visually proven linear/centered-radial fill overlays | Native subset; projected overlays recover typed CSS and guarded authored intent without repeated-cycle stop expansion | fidelity corpus + `cap:gradient-fill-overlay` + `cap:radial-gradient-fill-overlay` (both), LO + Graph evidence | every gradient form, transforms/stops, theme-aware stops, non-centered/focal/path variants, and broader renderer calibration |
| Picture fills and crop | Native, including one uniform solid overlay layer with independently validated base-image size/position/repeat | Native with exact base picture/crop and typed overlay regeneration | `cap:picture-crop` + `cap:picture-fill-overlay` (both), LO + Graph evidence | tile/contain/position variants, additional image effects, adversarial crop/transform cases |
...
| Outer shadow | Partial/native, including signed spread, calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, one outer-plus-inner `effectLst`, multiple CSS outer shadows as one editable flat sibling `effectDag`, and longer mixed lists as an exact owned layer above a schema-valid editable subset | Native with inverse CSS blur calibration, signed spread recovery, exact typed HTML container/source-reference/projection payload, CSS paint-order recovery across schema ordering, guarded full-intent recovery, path-aware custom-geometry CSS, and exact owned mixed-shadow custom paths | `cap:negative-shadow-spread` + `cap:effects` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` + `cap:custom-path-effects` + `cap:custom-path-owned-fallback` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line`, LO + Graph evidence | mixed shadows on pictures, thick/compound line alignment, broader color models, and any future renderer-proven native-only mixed graph |
| Inner shadow | Hybrid editable `a:innerShdw` plus one shape-bound portable fallback, alone, paired with one outer shadow, represented in a legal native subset beneath an exact longer mixed-shadow layer, or attached to custom geometry as one exact SVG-owned image layer; exact signed CSS intent and authored order are retained in private metadata and ignored when stale after an Office edit | Native read, browser-visible inset CSS for box geometry, exact intent/order/projection recovery, and regenerated portable fallback including custom paths | `cap:effects` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` (both) + `cap:custom-path-owned-fallback` (reverse) + native-projection mutation units, LO + Graph evidence | pictures, broader mixed effect families, and more renderer versions |
| Glow | Partial/native, including calibrated zero-offset single-path SVG `drop-shadow()` | CSS plus exact typed HTML payload and path-aware custom-geometry CSS | `cap:effects` + `cap:custom-path-effects` (both), LO + Graph evidence | compound effects, broader non-rect geometry, and halo calibration |
| Preset shadow | No HTML authoring mapping; exact source re-emits in a native whole-sequence PowerPoint branch, with a sole-node branch retained for the atomic case | Exact native source plus full-slide rasterized/noneditable fallback; multi-visual slides keep supported siblings in IR beneath one slide-owned image so incompatible renderers do not double-paint z-order | `cap:preset-shadow-raster-fallback` + `cap:preset-shadow-slide-fallback` (reverse, two normalized HTML cycles), overlapping translucent sibling regression, LO + Graph evidence | typed/editable semantics for proven presets, multiple preset shadows/unknown siblings, smaller independently owned isolation, all 20 presets, combinations, and real-deck coverage |
| Blur | Native PowerPoint effect retained beneath an isolated portable layer; strict lone SVG blur uses the same native `a:blur` beneath one exact custom-path-owned layer; unique compound `effectLst` children serialize in schema order | Native effect and fallback recovered as one hybrid with CSS, exact typed payload, and stable custom-path ownership | `cap:blur-effect` + `cap:compound-effect-list` + `cap:custom-path-blur` (both), LO + Graph evidence | blur-bearing and heterogeneous `effectDag` graphs, images, compound custom-geometry filters, and more renderer versions |
| Reflection | Native PowerPoint effect retained beneath an isolated portable layer for the conservative CSS `below`/pixel-gap/alpha-gradient subset, including strict lone custom-path reflection; unsupported CSS forms use the element-layer path | Native effect and fallback recovered as one hybrid; zero-blur effects use CSS reflection, independent reflection blur uses an owned mirrored layer, and custom paths retain exact typed ownership | `cap:reflection-effect` + `cap:custom-path-reflection` (both), LO + Graph evidence + blurred/rotated integration | other directions, compound ordering, pictures, compound custom-geometry filters, and more renderer versions |
| Soft edge | Native PowerPoint effect retained beneath one shape-bound portable layer for strict box/ellipse masks and the strict clipped-Gaussian custom-path pipeline; nondefault masks and changed SVG filter graphs use the element-layer path | Native effect and fallback recovered as one hybrid; valid rectangular CSS, ellipse-aware radial CSS, exact typed payload, and stable custom-path ownership | `cap:soft-edge-effect` + `cap:custom-path-soft-edge` (both) + OfficeCLI effects real deck, LO + Graph evidence | compound ordering, pictures, additional custom-geometry filter forms, remaining non-ellipse calibration, ellipse internal text rectangle, and more renderer versions |
| Fill overlay | Native PowerPoint solid, linear-gradient, or centered-radial metadata uses a shape-bound portable fallback selected for incompatible renderers for CSS multiply/screen/darken/lighten, including one picture base layer and a strict clipped solid-sRGB SVG graph for custom paths. One calibrated opaque horizontal multiply pattern retains hidden editable native `a:pattFill` metadata beneath one exact picture in the PowerPoint choice. A strict CSS `normal` overlay over a plain, untransformed, fully opaque solid square-cornered rectangle retains guarded native `over` intent beneath one exact picture selected in both renderer branches because Graph uses different compositing semantics and LibreOffice omits the native effect. Element opacity, additional effects, borders/text, partial geometry, active lower blends, non-centered radial forms, uncalibrated patterns, changed projections/filter graphs, ambiguous fills, and unsupported blends use the element-layer or preserved-source path | Four native-equivalent blend modes recover solid or gradient CSS; the calibrated pattern subset recovers horizontal multiply CSS. The strict authored normal subset recovers guarded typed `over` intent only while its native projection, hidden state, base composite, geometry, native/fallback bounds, and single-picture fallback branch remain unchanged, with a stable exact owned picture authoritative for visible paint. Admitted isolated square-cornered opaque source `over` rectangles retain a masked owned layer and exact source in `AlternateContent`, while inseparable crops report rasterized/noneditable | `cap:fill-overlay-effect` + `cap:gradient-fill-overlay` + `cap:radial-gradient-fill-overlay` + `cap:pattern-fill-overlay` + `cap:over-fill-overlay` + `cap:picture-fill-overlay` + `cap:custom-path-fill-overlay` (both) + `cap:fill-overlay-owned-fallback` (reverse, two normalized HTML cycles) + rejection/stale-intent/overlapping-crop regressions + Aspose real deck, LO + Graph evidence | broader pattern presets and blends, compound ordering, non-centered radial forms, additional custom-geometry gradient/filter forms, and more renderer versions |
| Effect containers and other constructs | Supported unique `effectLst` effects serialize in fixed ECMA order, including one inner-plus-outer pair; repeated mixed slots use a deterministic legal subset beneath one exact owned layer; repeatable outer shadows serialize as a flat sibling `effectDag` in reversed paint order with a final `fill` or `fillLine` source reference | Supported `effectLst` and flat sibling-shadow graph subsets are typed; authored order and exact projection survive schema normalization through guarded intent; arbitrary/nested graphs preserve as one attached container and use a source-owned renderer fallback when supplied | `cap:compound-effect-list` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both) + `cap:nested-effect-dag-fallback` (reverse) + arbitrary-graph fallback units, LO + Graph evidence | typed/editable heterogeneous tree graphs, duplicate non-shadow effects, smaller source-owned isolation, mixed picture/custom-geometry effects, and representative real decks |
...
| Tables | Native subset | Native subset; normalized HTML carries typed exact geometry plus table-style references/flags beside semantic markup | `cap:table` (both) + LO/Graph real deck | complete non-default styles/inheritance, borders, layout, nested content and adversarial cases |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 37 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and seven two-cycle reverse-first preservation/fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |

## Shared Work Remaining For PPTX

The dependency order is:

1. Expand strict group ownership/reconstruction beyond single-level untransformed native shapes and
   extend the chart-owned preservation contract to all preserved visuals; slide-scoped IDs, active
   provenance, chart graph ownership, and honest composite fallback for broader native groups are
   implemented.
