...
| Linear/radial gradients | Native subset; aspect-corrected DrawingML projection and premultiplied-sRGB stop subdivision are shared by shape fills and gradient fill overlays | Native subset; projected overlay gradients recover typed CSS and guarded authored intent without repeated-cycle stop expansion | fidelity corpus + `cap:gradient-fill-overlay` (both), LO + Graph evidence | every gradient form, transforms/stops, theme-aware stops, focal/path variants, and broader renderer calibration |
