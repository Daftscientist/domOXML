...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 28 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Effect containers and other constructs | Supported unique `effectLst` effects serialize in fixed ECMA order; repeatable outer shadows serialize as a flat sibling `effectDag` in reversed paint order with a final source reference | Supported `effectLst` and flat sibling-shadow graph subsets are typed; arbitrary/nested graphs preserve as one attached container and use a source-owned renderer fallback when supplied | `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` (both) + arbitrary-graph fallback units, LO + Graph evidence | heterogeneous/tree graphs, named containers/references, duplicate non-shadow effects, smaller source-owned isolation, and representative real decks |
