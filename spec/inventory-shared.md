...
| Fill overlay | Native PowerPoint solid, linear-gradient, or centered-radial metadata uses a shape-bound portable fallback selected for incompatible renderers for CSS multiply/screen/darken/lighten, including one picture base layer and a strict clipped solid-sRGB SVG graph for custom paths. One calibrated opaque horizontal multiply pattern retains hidden editable native `a:pattFill` metadata beneath one exact picture in the PowerPoint choice. A strict CSS `normal` overlay over an opaque solid square rectangle retains guarded native `over` intent beneath one exact picture selected in both renderer branches because Graph uses different compositing semantics and LibreOffice omits the native effect. Partial geometry, active lower blends, non-centered radial forms, uncalibrated patterns, changed projections/filter graphs, ambiguous fills, and unsupported blends use the element-layer or preserved-source path | Four native-equivalent blend modes recover solid or gradient CSS; the calibrated pattern subset recovers horizontal multiply CSS. The strict authored normal subset recovers guarded typed `over` intent only while its native projection is unchanged, with a stable exact owned picture authoritative for visible paint. Admitted isolated square-cornered opaque source `over` rectangles retain a masked owned layer and exact source in `AlternateContent`, while inseparable crops report rasterized/noneditable | `cap:fill-overlay-effect` + `cap:gradient-fill-overlay` + `cap:radial-gradient-fill-overlay` + `cap:pattern-fill-overlay` + `cap:over-fill-overlay` + `cap:picture-fill-overlay` + `cap:custom-path-fill-overlay` (both) + `cap:fill-overlay-owned-fallback` (reverse, two normalized HTML cycles) + rejection/stale-intent/overlapping-crop regressions + Aspose real deck, LO + Graph evidence | broader pattern presets and blends, compound ordering, non-centered/focal radial forms, additional custom-geometry gradient/filter forms, and more renderer versions |
