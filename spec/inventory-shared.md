...
|---|---|---|---|---|
| OPC packages and relationships | Native; generated/re-emitted output is blocked on content-type, relationship graph/reference, XML, and core-format validation | Native; source packages receive shared OPC validation | 21 focused package tests + capability package assertions + 36 capabilities + 7 real decks | full XSD/Open XML validation, strict packages, general alternate content, extension schemas, general extension re-emission beyond attached charts |
...
| Custom paths | Partial/native; single-path SVG drop shadow/glow effects are native, while strict lone SVG blur and below-reflection are hybrid native/exact | Partial/native; DrawingML guides and elliptical arcs normalize to canonical path commands, with path-aware effect HTML, exact typed geometry/effect metadata, and stable exact owned layers for mixed shadows, blur, and reflection | `cap:custom-path` + `cap:custom-path-effects` + `cap:custom-path-blur` + `cap:custom-path-reflection` (both) + `cap:custom-path-owned-fallback` (reverse) + Apache POI real deck, LO + Graph evidence | authored SVG arcs/shorthand, multipath, guide/source-formula retention, multi-path fill/stroke semantics, remaining effect families and compound filters |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 30 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and five two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
