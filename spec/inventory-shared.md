...
| Custom paths | Partial/native; single-path SVG drop shadow/glow effects are native | Partial/native; DrawingML guides and elliptical arcs normalize to canonical path commands, with path-aware effect HTML and exact typed geometry/effect metadata | `cap:custom-path` + `cap:custom-path-effects` (both) + Apache POI real deck, LO + Graph evidence | authored SVG arcs/shorthand, multipath, guide/source-formula retention, multi-path fill/stroke semantics, remaining effect families and compound filters |
...
| Outer shadow | Partial/native, including signed spread, calibrated single-path SVG `drop-shadow()`, schema-ordered blur-plus-shadow lists, one outer-plus-inner `effectLst`, multiple CSS outer shadows as one editable flat sibling `effectDag`, and longer mixed lists as an exact owned layer above a schema-valid editable subset | Native with inverse CSS blur calibration, signed spread recovery, exact typed HTML container/source-reference/projection payload, CSS paint-order recovery across schema ordering, guarded full-intent recovery, and path-aware custom-geometry CSS | `cap:negative-shadow-spread` + `cap:effects` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` + `cap:custom-path-effects` + `cap:compound-effect-list` + `cap:multiple-shadow-effect-dag` + `cap:effect-dag-fill-line` (both), LO + Graph evidence | mixed shadows on pictures/custom geometry, thick/compound line alignment, broader color models, and any future renderer-proven native-only mixed graph |
| Inner shadow | Hybrid editable `a:innerShdw` plus one shape-bound portable fallback, alone, paired with one outer shadow, or represented in a legal native subset beneath an exact longer mixed-shadow layer; exact signed CSS intent and authored order are retained in private metadata and ignored when stale after an Office edit | Native read, browser-visible inset CSS, exact intent/order/projection recovery, and regenerated portable fallback | `cap:effects` + `cap:mixed-shadow-effect-list` + `cap:mixed-shadow-owned-fallback` (both) + native-projection mutation units, LO + Graph evidence | pictures/custom geometry, broader mixed effect families, and more renderer versions |
...
| Repeated round trips | every reverse-capable fixture rebuilds and re-ingests at least twice with representation/editability/retention/output/raster bounds | 28 bidirectional fixtures, a three-cycle scoped reverse-first chart fixture, and four two-cycle reverse-first fallback fixtures | capability runner + manifest contract tests | broader source-format preservation gates and PowerPoint-rendered convergence |
