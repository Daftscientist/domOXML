...
| Preset shadow | No HTML authoring mapping; retained native source re-emits in the PowerPoint branch | Exact native source plus full-slide rasterized/noneditable fallback selected for incompatible renderers on a sole-visual slide | `cap:preset-shadow-raster-fallback` (reverse, two normalized HTML cycles) + multi-visual z-order regression, LO + Graph evidence | typed/editable semantics for proven presets, slide-level or smaller independently owned isolation, all 20 presets, combinations, and real-deck coverage |
